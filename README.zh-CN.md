...
- 🎨 **Aurora Glass 主题**（2026 设计语言）：深空底色上的液态玻璃表面，双色极光——品牌金（火）× 电光蓝（冰）——胶片颗粒质感、超大渐变字体排版、渐变金银铜排名数字；刻意与 Steam 官方界面区分开
...
- 📈 **24h 在线趋势线**：最热游玩每行带一条滚动 24 小时在线迷你曲线，按走势着色（绿涨/红跌/金平；服务运行期间内存采样，完整数据在 `/api/history/{appid}`）——FastAPI 端专属
- 📡 **今日速览条**：标题下一行看全——此刻总在线、🔥 异动游戏（30 分钟 vs 2 小时基线）、新上榜数、限时免费进行中；每次打开 3 秒知道今天有什么不一样
- ⭐ **关注列表**（FastAPI 端）：点游戏名前的 ☆ 关注，一键「★ 我的关注」在全部榜单只看星标；关注的游戏折扣加深时星星变绿提醒
- ✨ **克制的微动效**：在线数字变化时闪金色、封面 hover 微放大、异动徽章呼吸——刻意轻微，不破坏 Aurora 的安静感
